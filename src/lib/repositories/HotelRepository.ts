...
export class HotelRepository {
  ...
  private async upsertOneHotel(params: {
    offer: HotelOffer;
    cityId: number;
    dates: Date[];
    generatedAt: Date;
    ...
    const { offer, cityId, dates, generatedAt, checkinDate, checkoutDate } =
      params;
    ...
    try {
      ...
      await Promise.all([
        this.upsertCancellationPolicy(hotel.id, offer.cancellationPolicy),
        this.upsertHotelAmenities(hotel.id, offer.amenities),
        Promise.all(
          offer.roomTypes.map((rt) =>
            this.upsertRoomTypeWithAvailability({
              hotelId: hotel.id,
              rt,
              dates,
              generatedAt,
  ...
  private async upsertCancellationPolicy(
    hotelId: number,
    policy: HotelOffer['cancellationPolicy'],
  ): Promise<void> {
    ...
    await Promise.all(
      amenityRows.map(({ id: amenityId }) =>
        this.prisma.hotelAmenity.upsert({
          where: {
            hotelId_amenityId: { hotelId, amenityId },
          },
  ...
  private async upsertRoomTypeWithAvailability(params: {
    hotelId: number;
    rt: HotelOffer['roomTypes'][number];
    dates: Date[];
    generatedAt: Date;
  }): Promise<void> {
    const { hotelId, rt, dates, generatedAt } = params;
    ...
    const roomType = await this.prisma.roomType.upsert({
      where: { hotelId_name: { hotelId, name: rt.name } },
      create: {
        hotelId,
        name: rt.name,
