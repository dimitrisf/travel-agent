  ...
  async proposeBooking(
    ...
    try {
      return await this.prisma.$transaction(async (tx) => {
        ...
        //     },
        //   ]
        const hotelRanges = parsed.hotels.map((stay) => {
        ...
        const flightInstances =
          parsed.flights.length > 0
            ? await tx.flightInstance.findMany({
                where: {
                  id: {
                    in: parsed.flights.map((f) => f.flight_instance_id),
                  },
                },
        ...
        //   }
        const flightById = new Map(flightInstances.map((fi) => [fi.id, fi]));
        ...
        const roomTypes =
          parsed.hotels.length > 0
            ? await tx.roomType.findMany({
                where: {
                  id: {
                    in: parsed.hotels.map((h) => h.room_type_id),
                  },
                },
              })
            : [];
