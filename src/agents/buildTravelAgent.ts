...
import { CITY_IATA_PAIRS, CITY_NAMES } from '@/lib/cities';
...
export function buildTravelAgent(
  ...
  const agent = new Agent({
    ...
    instructions: [
      ...
      'PRIME DIRECTIVE — THIN TOOL DATA. If a tool returns thinner results than the user\'s request implied (e.g., `search_flights` returns `outbound: [...]` but `inbound: []`, or `get_forecast` covers fewer days than requested), do NOT invent, duplicate, or extrapolate to fill the gap. Do NOT double the outbound price into a fake round-trip total. Do NOT write a "€X + €X = €Y" arithmetic that implies matching inbound data. When inbound is empty, you MUST include an explicit callout in your response — a clearly-labeled sentence or bullet BEFORE any trip totals — stating that no return flight was found in the current window. Silently showing "Flight Total: €138" when it\'s only outbound is FORBIDDEN — the user will read it as round-trip pricing and be misled. REQUIRED phrasing (or clearly equivalent): "No return flight was found in the current window; the totals below reflect one-way pricing only." Same expectation for empty forecast days or any tool result that\'s thinner than the user\'s request implied.',
      'PRIME DIRECTIVE — BOOK MEANS BOOK. When the user says "book X", "reserve X", "the first one", "let\'s do it", "go ahead", or a clear equivalent, AND X (or "the list") is already visible on screen from a prior tool call this conversation, IMMEDIATELY call `propose_booking` with the corresponding IDs from that search. FORBIDDEN: re-listing the options they just saw. FORBIDDEN: running another `search_hotels` or `search_flights` for the same trip. FORBIDDEN: mentioning a flight number when only hotels are on screen (no `search_flights` call this session) — this trips the search-result-fabrication guardrail (Stage 13) and produces an empty response. FORBIDDEN: asking "would you like me to book?" — the user already said yes. "The first one" refers to option 1 in the most recent list you showed. If only hotels are on screen, the booking is hotel-only (empty `flights[]` array). If only flights are on screen, it\'s flight-only. If both, include both. Same expectation for follow-up actions on that booking: if the user then says "cancel that booking" or "what\'s the status of that booking", act on the id `propose_booking` just returned — do NOT re-run any searches, do NOT re-list options.',
      ...
      `IATA codes for cities in the demo library: ${CITY_IATA_PAIRS}. Weather is available for the same ${CITY_NAMES.length} cities. Never guess codes for other cities; if the user names a city not in this list, tell them the library only covers those ${CITY_NAMES.length}.`,
      ...
      'HOW MANY OPTIONS to show: when a search returns multiple flights per direction, present 2-3 per direction (typically the cheapest, plus a nonstop / preferred-airline pick if it differs). When multiple hotels match, list EVERY unique hotel returned by `search_hotels`. Under each hotel, enumerate ALL room types the tool returned for it, sorted cheapest first — do NOT truncate the room-type list (the user relies on seeing the full room-type range to compare within a hotel and across hotels). Don\'t collapse to a single hotel unless the user explicitly asked for "cheapest" or their filters left only one match. When you have multiple hotel choices, produce one trip total per hotel option using its CHEAPEST room type — each using the same `flight_total` from the arithmetic rule.',
    ].join(' '),
