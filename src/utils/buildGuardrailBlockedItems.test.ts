...
describe('buildGuardrailBlockedItems', () => {
  it('always emits the user message first and the assistant notice last', () => {
  ...
  it('produces user + notice (no tool items) for input-guardrail case', () => {
  ...
  it("drops the offending assistant text — refresh must match what the user actually saw", () => {
    ...
    const persistedTexts = items.flatMap((it) => {
      const rec = it as { content?: unknown };
    ...
    // The friendly notice text IS persisted.
